...
async function initMercadoPago() {
  try {
    ...
    const response = await fetch(`${baseUrl}/payment-status.php?mercadopago-config`);
    if (response.ok) {
  ...
  try {
    ...
    if (response.ok) {
      ...
      document.getElementById('check-payment-status').addEventListener('click', async function() {
        ...
        try {
          const statusResponse = await fetch(`${baseUrl}/payment-status.php?payment-status/${data.payment_id}`);
